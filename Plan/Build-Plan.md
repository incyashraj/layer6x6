...
| 3 | UI + Graphics | Started | 2026-05-19 | pending | Phase 3 has begun at the contract and prototype layer: draft GUI WIT, GUI manifest recognition, first `ui`/`gfx`/`audio` capability names, shared UI adapter model, runtime UI dispatcher scaffold, layout proof, input routes, and an opt-in macOS AppKit window prototype with event bridge, session state, delegate-shaped native event state, redraw bridge, delegate callback bridge, draw-surface state, draw view surface, retained native window delegate, non-blocking event-loop step driver, and selectable native prototype runtime mode are in place. |
