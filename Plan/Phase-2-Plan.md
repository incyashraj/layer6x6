...
| 5 | TypeScript (jco) bindings generated and usable; sample builds and runs | Started: `layer36 doctor` reports Node/npm/jco readiness, `packages/sdk-ts` has the SDK scaffold and shape checks, and CI/self-hosted now run a fixture-build step that auto-builds TypeScript variant components when jco is available; full always-on hosted runtime proof remains |
...
| SDKs | Rust is usable; Go scaffold is ready; TypeScript scaffold plus first fixture build lane is ready | Publish-ready Rust after freeze, TinyGo runtime proof, and always-on jco runtime proof on hosted CI. |
