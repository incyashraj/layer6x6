...
|------|--------------|--------------|
| Core Phase 2 engineering | About 67-72% through the first useful CLI slice | WIT freeze review, more shared adapter slices, cross-host identity runs, and network hardening. |
...
| P2-ADPT-COMMON-02 | Expand shared adapter-common beyond HTTP framing | 2026-05-04 | The first path normalization slice now exists. Sandbox-root resolution, time arithmetic, ICU/locale helpers, and the final per-OS adapter split remain before the adapter-common exit box can be checked. |
